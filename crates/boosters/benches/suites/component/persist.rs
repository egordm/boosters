...
use boosters::persist::{BinaryReadOptions, BinaryWriteOptions, SerializableModel};
...
fn build_tree_with_nodes(target_nodes: usize) -> boosters::repr::gbdt::Tree<ScalarLeaf> {
    ...
    let depth = depth.max(1).min(10); // Clamp to reasonable depth
...
fn build_subtree(builder: &mut MutableTree<ScalarLeaf>, node_id: u32, feature: u32, remaining_depth: u32) {
...
fn bench_read_model(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("binary", "100_trees"), |b| {
        b.iter(|| {
            ...
            let loaded =
                GBDTModel::read_from(cursor, &BinaryReadOptions::default()).unwrap();
            black_box(loaded)
        });
    });
...
fn bench_inspect_header(c: &mut Criterion) {
    ...
    group.bench_function("header_only", |b| {
        b.iter(|| {
            let cursor = Cursor::new(&buf);
            // Currently we don't have a header-only read, so this is a placeholder
            // TODO: Add Envelope::read_header() when implemented
            let loaded =
                GBDTModel::read_from(cursor, &BinaryReadOptions::default()).unwrap();
            black_box((loaded.meta().n_features, loaded.forest().n_trees()))
        });
    });
