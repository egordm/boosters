...
/// Iterator that yields weights (or 1.0 if unweighted).
/// 
...
pub(crate) enum WeightIter<'a> {
    Unweighted { remaining: usize },
    Weighted { weights: ndarray::ArrayView1<'a, f32>, idx: usize },
}
...
impl<'a> Iterator for WeightIter<'a> {
    ...
    fn next(&mut self) -> Option<f32> {
        match self {
            WeightIter::Unweighted { remaining } => {
                if *remaining > 0 {
                    *remaining -= 1;
                    Some(1.0)
                } else {
            ...
            WeightIter::Weighted { weights, idx } => {
                if *idx < weights.len() {
                    let val = weights[*idx];
                    *idx += 1;
                    Some(val)
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let len = match self {
            WeightIter::Unweighted { remaining } => *remaining,
            WeightIter::Weighted { weights, idx } => weights.len() - idx,
        };
        (len, Some(len))
    }
}

impl<'a> ExactSizeIterator for WeightIter<'a> {}

/// Returns an iterator over weights, using 1.0 for empty/None weights.
///
...
/// 
/// The branch on weighted vs unweighted is done once at construction,
/// not on every iteration.
///
/// # Example
///
/// ```ignore
/// for ((&target, &pred), w) in targets.iter()
///     .zip(preds.iter())
...
/// ```
#[inline]
pub(crate) fn weight_iter(
    weights: Option<ndarray::ArrayView1<'_, f32>>,
    n_rows: usize,
) -> WeightIter<'_> {
    match weights {
        Some(w) => WeightIter::Weighted { weights: w, idx: 0 },
        None => WeightIter::Unweighted { remaining: n_rows },
    }
}
...
mod tests {
    ...
    #[test]
    fn test_weight_iter_none() {
        let result: Vec<f32> = weight_iter(None, 3).collect();
    ...
    fn test_weight_iter_with_weights() {
        ...
        let result: Vec<f32> = weight_iter(Some(weights.view()), 3).collect();
