...
impl<'f, T: TreeTraversal<ScalarLeaf>> Predictor<'f, T> {
    ...
    /// Predict from any [`DataAccessor`] into provided buffer.
    ///
    /// This method works with any data source that implements `DataAccessor`,
    /// including `BinnedDataset`, `SamplesView`, or custom types.
    ///
    /// # Arguments
    ///
    /// * `data` - Data accessor providing sample-by-sample access
    /// * `tree_weights` - Optional per-tree weights for DART dropout
    /// * `output` - Output buffer `[n_groups, n_samples]`
    ///
    /// # Performance Note
    ///
    /// For `BinnedDataset`, each sample access may involve lookups rather than
    /// contiguous memory access. For maximum performance with large datasets,
    /// consider using `SampleBlocks` to convert to contiguous blocks first.
    pub fn predict_from_into<D: DataAccessor>(
        &self,
        data: &D,
        tree_weights: Option<&[f32]>,
        mut output: ArrayViewMut2<f32>,
        ...
        // Predict using DataAccessor interface
        self.predict_block_from(data, tree_weights, output);
    }

    /// Convenience method: predict from DataAccessor with allocation.
    ///
    /// # Arguments
    ///
    /// * `data` - Data accessor (e.g., `BinnedDataset`)
    ///
    /// # Returns
    ///
    /// Prediction matrix `[n_groups, n_samples]`
    pub fn predict_from<D: DataAccessor>(&self, data: &D) -> Array2<f32> {
        let n_samples = data.n_samples();
        let n_groups = self.n_groups();
        let mut output = Array2::<f32>::zeros((n_groups, n_samples));
        self.predict_from_into(data, None, output.view_mut());
        output
    }

    /// Internal: predict a block using DataAccessor interface.
    fn predict_block_from<D: DataAccessor>(
        &self,
        data: &D,
        tree_weights: Option<&[f32]>,
        mut output: ArrayViewMut2<f32>,
        ...
        for (tree_idx, (tree, group)) in self.forest.trees_with_groups().enumerate() {
            ...
            // Step 1: Get leaf indices via DataAccessor
            T::traverse_block(tree, state, data, &mut leaf_indices);
...
mod tests {
    ...
    // =========================================================================
    // DataAccessor / BinnedDataset prediction tests
    // =========================================================================

    #[test]
    fn predict_from_matches_predict() {
        ...
        let sample_data: Vec<f32> = vec![0.2, 0.4, 0.6, 0.8];
        let sample_view = ArrayView2::from_shape((4, 1), &sample_data).unwrap();
        let samples = SamplesView::from_array(sample_view);

        // predict_from uses DataAccessor interface
        let output = predictor.predict_from(&samples);

        // Verify expected values
        // 0.2 < 0.5 -> left (1.0 + 0.5 = 1.5)
        // 0.4 < 0.5 -> left (1.0 + 0.5 = 1.5)
        // 0.6 >= 0.5 -> right (2.0 + 1.5 = 3.5)
        // 0.8 >= 0.5 -> right (2.0 + 1.5 = 3.5)
        assert_eq!(output.shape(), &[1, 4]);
        assert_abs_diff_eq!(output[[0, 0]], 1.5, epsilon = 1e-6);
        assert_abs_diff_eq!(output[[0, 1]], 1.5, epsilon = 1e-6);
        assert_abs_diff_eq!(output[[0, 2]], 3.5, epsilon = 1e-6);
        assert_abs_diff_eq!(output[[0, 3]], 3.5, epsilon = 1e-6);
    }

    #[test]
    fn predict_from_binned_dataset() {
        use crate::data::binned::BinnedDatasetBuilder;
        use ndarray::arr1;
        ...
        // Create BinnedDataset with 4 samples, 1 feature
        // Feature values: [0.2, 0.4, 0.6, 0.8]
        let raw_data = arr1(&[0.2f32, 0.4, 0.6, 0.8]);
        let dataset = BinnedDatasetBuilder::new()
            .add_numeric("feature0", raw_data.view())
            .build()
            .expect("build should succeed");

        // Verify BinnedDataset has the right data
        assert_eq!(dataset.n_samples(), 4);
        assert_eq!(dataset.n_features(), 1);

        // Use predict_from with BinnedDataset
        let output = predictor.predict_from(&dataset);

        // Verify expected values (same as above)
        // 0.2 < 0.5 -> left (1.0 + 0.5 = 1.5)
        // 0.4 < 0.5 -> left (1.0 + 0.5 = 1.5)
        // 0.6 >= 0.5 -> right (2.0 + 1.5 = 3.5)
        // 0.8 >= 0.5 -> right (2.0 + 1.5 = 3.5)
    ...
    #[test]
    fn predict_from_binned_multifeature() {
        use crate::data::binned::BinnedDatasetBuilder;
        use ndarray::arr1;

        // Deeper tree that uses 2 features
        let mut forest = Forest::for_regression();
        forest.push_tree(build_deeper_tree(), 0);

        let predictor = UnrolledPredictor6::new(&forest);

        // Create BinnedDataset with 4 samples, 2 features
        // Feature 0: [0.2, 0.2, 0.6, 0.6]  (threshold 0.5)
        // Feature 1: [0.1, 0.5, 0.5, 0.9]  (threshold 0.3 for left, 0.7 for right)
        let feature0 = arr1(&[0.2f32, 0.2, 0.6, 0.6]);
        let feature1 = arr1(&[0.1f32, 0.5, 0.5, 0.9]);

        let dataset = BinnedDatasetBuilder::new()
            .add_numeric("feature0", feature0.view())
            .add_numeric("feature1", feature1.view())
            .build()
            .expect("build should succeed");

        // Use predict_from with BinnedDataset
        let binned_output = predictor.predict_from(&dataset);

        // Also compute via FeaturesView for comparison
        // Feature-major: [n_features=2, n_samples=4]
        let feature_major_data = vec![
            0.2, 0.2, 0.6, 0.6, // feature 0 values
            0.1, 0.5, 0.5, 0.9, // feature 1 values
        ];
        let features = features_view(&feature_major_data, 2, 4);
        let features_output = predictor.predict(features, Parallelism::Sequential);

        // Both should produce identical results
        assert_abs_diff_eq!(binned_output, features_output, epsilon = 1e-6);
    }
}
