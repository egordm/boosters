...
use boosters::training::{
    Accuracy, GrowthStrategy, LinearLeafConfig, LogLoss, Mae, Metric, MetricFn, MulticlassAccuracy,
...
use xgb::parameters::{
    ...
    learning::LearningTaskParametersBuilder, learning::Objective as XgbObjective,
};
#[cfg(feature = "bench-xgboost")]
...
fn default_configs(quick: bool) -> Vec<BenchmarkConfig> {
    let (small_rows, medium_rows) = if quick {
        (2_000, 10_000)
    } else {
        (10_000, 50_000)
    };
...
fn load_libsvm_dense(path: &Path) -> (Vec<f32>, Vec<f32>, usize, usize) {
    ...
    for (line_idx, line) in content.lines().enumerate() {
        ...
        let y: f32 = y_str
            .parse()
            .unwrap_or_else(|_| panic!("invalid label at line {}", line_idx + 1));
        labels.push(y);
        let mut feats: Vec<(usize, f32)> = Vec::new();
        for p in parts {
            let (idx_str, val_str) = p.split_once(':').unwrap_or_else(|| {
                panic!("invalid feature token '{}' at line {}", p, line_idx + 1)
            });
            let idx1: usize = idx_str
                .parse()
                .unwrap_or_else(|_| panic!("invalid feature index at line {}", line_idx + 1));
            if idx1 == 0 {
                panic!(
                    "libsvm feature indices must be 1-based (got 0) at line {}",
                    line_idx + 1
                );
            }
            let idx0 = idx1 - 1;
...
fn load_label0_dense(path: &Path) -> (Vec<f32>, Vec<f32>, usize, usize) {
    ...
    for (line_idx, line) in content.lines().enumerate() {
        ...
        assert!(
            parts.len() >= 2,
            "expected label + at least 1 feature at line {}",
            line_idx + 1
        );
...
fn train_boosters(
    ...
    let sample_major = ArrayView2::from_shape((rows_train, cols), x_train)
    ...
    let valid_targets_arr =
        Array2::from_shape_vec((1, y_valid.len()), y_valid.to_vec()).expect("valid shape");
    let valid_targets = TargetsView::new(valid_targets_arr.view());
...
fn train_lightgbm(
    ...
    let mut params = match config.task {
        ...
        Task::Multiclass => {
            serde_json::json!({"objective":"multiclass","metric":"multi_logloss","num_class": n_classes})
        }
    };
...
fn run_single_eval(config: &BenchmarkConfig, seed: u64) -> RunResult {
    let (x_train, y_train, rows_train, x_valid, y_valid, rows_valid, cols) =
        load_data(config, seed, 0.2);

    let boosters = train_boosters(
        config, &x_train, &y_train, rows_train, &x_valid, &y_valid, rows_valid, cols, seed,
    );
    ...
    let xgboost = if config.linear_leaves {
        ...
        Some(train_xgboost(
            config, &x_train, &y_train, rows_train, &x_valid, &y_valid, rows_valid, cols,
        ))
    };
...
fn run_benchmark(config: &BenchmarkConfig, seeds: &[u64]) -> BenchmarkResult {
    ...
    BenchmarkResult {
        ...
        xgboost: if xgb_runs.is_empty() {
            None
        } else {
            Some(AggregatedMetrics::from_runs(&xgb_runs))
        },
        lightgbm: if lgb_runs.is_empty() {
            None
        } else {
            Some(AggregatedMetrics::from_runs(&lgb_runs))
        },
    }
}
...
fn generate_report(results: &[BenchmarkResult], seeds: &[u64]) -> String {
    ...
    for task in ["regression", "binary", "multiclass"] {
        let Some(task_results) = by_task.get(task) else {
            continue;
        };
...
fn main() {
    ...
    for (i, path) in args.libsvm_paths.iter().enumerate() {
        let name = path
            .file_stem()
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| format!("libsvm_{}", i));
        configs.push(BenchmarkConfig {
    ...
    for (i, path) in args.label0_paths.iter().enumerate() {
        let name = path
            .file_stem()
            .map(|s| s.to_string_lossy().to_string())
            .unwrap_or_else(|| format!("label0_{}", i));
        configs.push(BenchmarkConfig {
    ...
    println!(
        "Running {} configurations with {} seeds each",
        configs.len(),
        seeds.len()
    );
    println!();
