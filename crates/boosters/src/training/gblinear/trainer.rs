...
use crate::training::{
    EarlyStopping, EarlyStopAction, EvalSet, Gradients, Metric, ObjectiveFn, ObjectiveFnExt, TrainingLogger, Verbosity,
};
...
/// ```
#[derive(Clone, Debug)]
pub struct GBLinearTrainer<O: ObjectiveFn, M: Metric> {
    objective: O,
    metric: M,
    params: GBLinearParams,
}

impl<O: ObjectiveFn, M: Metric> GBLinearTrainer<O, M> {
