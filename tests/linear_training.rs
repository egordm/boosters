...
use booste_rs::training::{
    Accuracy, HingeLoss, LogisticLoss, Metric, MulticlassAccuracy, PseudoHuberLoss, QuantileLoss,
    Rmse, SoftmaxLoss, SquaredLoss, Verbosity,
};
...
fn train_greedy_selector_feature_priority() {
    ...
    assert!(greedy_has_weights, "Greedy model should have non-zero weights");
    assert!(cyclic_has_weights, "Cyclic model should have non-zero weights");
...
// =============================================================================
// PseudoHuberLoss and HingeLoss Integration Tests
// =============================================================================
...
fn train_pseudo_huber_regression() {
    ...
    let model = LinearTrainer::new(config.clone())
        .train(&data, &labels, &PseudoHuberLoss::new(5.0)); // Larger slope
...
fn train_pseudo_huber_with_slope() {
    ...
    let moderate_slope_model = LinearTrainer::new(config.clone())
        .train(&data, &labels, &PseudoHuberLoss::new(2.0));

    // Train with large slope (more like squared)
    let large_slope_model = LinearTrainer::new(config.clone())
        .train(&data, &labels, &PseudoHuberLoss::new(10.0));
    ...
    assert!(moderate_rmse < 50.0, "Moderate slope RMSE too high: {}", moderate_rmse);
    assert!(large_rmse < 20.0, "Large slope RMSE too high: {}", large_rmse);

    // Large slope should be closer to squared loss
    let large_diff = (large_rmse - sq_rmse).abs();
    println!(
        "Large slope diff from squared: {:.4}",
        large_diff
    );
...
fn train_hinge_binary_classification() {
    ...
    let hinge_model = LinearTrainer::new(hinge_config.clone())
        .train(&data, &labels, &HingeLoss);

    // Also train with logistic for comparison
    let logistic_model = LinearTrainer::new(hinge_config)
        .train(&data, &labels, &LogisticLoss);
