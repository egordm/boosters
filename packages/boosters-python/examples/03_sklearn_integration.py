...
def main() -> None:
    ...
    y = features[:, 0] + 0.5 * features[:, 1] + rng.standard_normal(200).astype(np.float32) * 0.1

    # Cross-validation
    print("\n--- Cross-Validation ---")
    reg = GBDTRegressor(n_estimators=50, verbose=0)
    scores = cross_val_score(reg, features, y, cv=5, scoring="neg_root_mean_squared_error")
