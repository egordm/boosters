...
Maintenance
-----------
This stub file is manually maintained to provide accurate type hints.
When modifying Rust PyO3 bindings in `src/`, check that the corresponding
stub entries are updated here:

1. New class → Add class definition with all public attributes and methods
2. New method → Add method signature with correct types
3. Changed signature → Update parameter types, return type, and defaults
4. Removed class/method → Delete from this file

The pyo3-stub-gen crate provides the #[gen_stub_pyclass] attribute macro
which helps ensure type information is available, but full automation
is limited by PyO3 extension-module constraints.

Run `uv run poe typecheck` after changes to verify stubs are consistent.
"""
...
class TreeConfig:
    ...
    growth_strategy: Literal["leafwise", "depthwise"]
    ...
    def __init__(
        ...
        growth_strategy: Literal["leafwise", "depthwise"] = "depthwise",
    ) -> None: ...
...
class GBDTModel:
    ...
    def predict(
        self,
        features: NDArray[np.float32] | Dataset,
        ...
        Args:
            ...
            n_iterations: Number of trees to use (None = all trees).
...
class GBLinearModel:
    ...
    Uses gradient boosting to train a linear model via coordinate descent.
    ...
    def n_features_in_(self) -> int:
        """sklearn-compatible alias for n_features."""
        ...
    ...
    def coef_(self) -> NDArray[np.float32]:
        """Get model coefficients (sklearn-compatible).

        Returns:
            Array of shape (n_features,) for single output or
            (n_features, n_outputs) for multi-output.
    ...
    def intercept_(self) -> NDArray[np.float32]:
        """Get model intercept/bias (sklearn-compatible).

        Returns:
            Scalar or array of intercept values.
    ...
    def fit(
        ...
        Args:
            train: Training dataset containing features and labels.
            eval_set: Optional validation set(s) for early stopping.
    ...
    def predict(
        ...
        Args:
            features: Feature array of shape (n_samples, n_features) or Dataset.
            raw_score: If True, return raw margin scores.
