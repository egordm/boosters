...
Establish baseline performance characteristics for booste-rs vs XGBoost C++.
Answer: How fast is our simple row-by-row traversal compared to XGBoost?

## Summary

First benchmark run establishing baseline performance characteristics.
booste-rs is ~49x faster for single-row inference due to no DMatrix overhead.
XGBoost wins at large batches (1K+ rows) due to SIMD and block optimizations.
...
**Note**: XGBoost threading was not explicitly constrained in this benchmark.
XGBoost may have used multiple threads via OpenMP. See later benchmarks for
