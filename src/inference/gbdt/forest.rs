...
use super::{CategoriesStorage, LeafValue, ScalarLeaf, TreeStorage};
...
impl<L: LeafValue> Forest<L> {
    ...
    #[inline]
    pub fn tree(&self, idx: usize) -> TreeView<'_, L> {
        TreeView {
    ...
    pub fn trees(&self) -> impl Iterator<Item = TreeView<'_, L>> {
        self.trees.iter().map(|t| TreeView { storage: t })
    }

    /// Iterate over trees with their group assignments.
    pub fn trees_with_groups(&self) -> impl Iterator<Item = (TreeView<'_, L>, u32)> {
        self.trees
            .iter()
            .zip(self.tree_groups.iter())
            .map(|(t, &g)| (TreeView { storage: t }, g))
    }
}
...
pub struct TreeView<'a, L: LeafValue> {
    storage: &'a TreeStorage<L>,
}
...
impl<'a, L: LeafValue> TreeView<'a, L> {
    ...
    #[inline]
    pub fn split_type(&self, node_idx: u32) -> super::node::SplitType {
    ...
    #[inline]
    pub fn into_storage(self) -> &'a TreeStorage<L> {
        self.storage
    }
}
...
mod tests {
    use super::*;
    use crate::inference::gbdt::tree::TreeBuilder;

    /// Build a simple tree:
    ///        [0] feat0 < 0.5
    ///        /          \
    ///    [1] leaf=1.0   [2] leaf=2.0
    fn build_simple_tree(left_val: f32, right_val: f32, threshold: f32) -> TreeStorage<ScalarLeaf> {
    ...
    fn tree_view_delegates_to_storage() {
        ...
        let forest = {
            let mut f = Forest::for_regression();
            f.push_tree(tree, 0);
            f
        };
