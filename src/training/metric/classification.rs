            ...
            Some(w) => {
                ...
                let (weighted_correct, weight_sum) = predictions
                    ...
                    .fold((0.0f64, 0.0f64), |(acc_correct, acc_w), ((p, l), wt)| {
                        let wt = *wt as f64;
                        let is_correct =
                            if (p.round() - l.round()).abs() < 0.5 { 1.0 } else { 0.0 };
                        (acc_correct + wt * is_correct, acc_w + wt)
                    });
...
impl Metric for Auc {
    fn evaluate(
        ...
        match weights {
            None => compute_auc_unweighted(predictions, labels),
            Some(w) => {
                debug_assert_eq!(predictions.len(), w.len());
                compute_auc_weighted(predictions, labels, w)
            }
        }
    }
...
mod tests {
    ...
    fn logloss_perfect() {
        ...
        let ll = LogLoss.evaluate(&preds, &labels, None, 1);
    ...
    fn weighted_logloss_emphasizes_high_weight_samples() {
        ...
        let unweighted = LogLoss.evaluate(&preds, &labels, None, 1);

        // With high weight on sample 0 (good prediction): lower loss
        let weights_good = vec![10.0, 1.0];
        let weighted_good = LogLoss.evaluate(&preds, &labels, Some(&weights_good), 1);

        // With high weight on sample 1 (bad prediction): higher loss
        let weights_bad = vec![1.0, 10.0];
        let weighted_bad = LogLoss.evaluate(&preds, &labels, Some(&weights_bad), 1);
    ...
    fn accuracy_perfect() {
        ...
        let acc = Accuracy::default().evaluate(&preds, &labels, None, 1);
    ...
    fn accuracy_custom_threshold() {
        ...
        let acc_05 = Accuracy::default().evaluate(&preds, &labels, None, 1);
        assert!((acc_05 - 0.5).abs() < 1e-10);

        // With threshold 0.2, all predictions are 1
        let acc_02 = Accuracy::with_threshold(0.2).evaluate(&preds, &labels, None, 1);
    ...
    fn weighted_accuracy_emphasizes_correct_samples() {
        ...
        let unweighted = Accuracy::default().evaluate(&preds, &labels, None, 1);
        assert!((unweighted - 0.5).abs() < 1e-10);

        // High weight on correct sample: higher accuracy
        let weights_correct = vec![10.0, 1.0];
        let weighted_correct = Accuracy::default().evaluate(&preds, &labels, Some(&weights_correct), 1);
        ...
        let weighted_incorrect =
            Accuracy::default().evaluate(&preds, &labels, Some(&weights_incorrect), 1);
    ...
    fn multiclass_accuracy() {
        ...
        let acc = MulticlassAccuracy.evaluate(&preds, &labels, None, 1);
    ...
    fn weighted_multiclass_accuracy() {
        ...
        let unweighted = MulticlassAccuracy.evaluate(&preds, &labels, None, 1);
        assert!((unweighted - 0.75).abs() < 1e-10);

        // High weight on the wrong sample: accuracy drops
        let weights = vec![1.0, 1.0, 1.0, 10.0];
        let weighted = MulticlassAccuracy.evaluate(&preds, &labels, Some(&weights), 1);
    ...
    fn auc_perfect() {
        ...
        let auc = Auc.evaluate(&preds, &labels, None, 1);
    ...
    fn auc_matches_naive_implementation() {
        ...
        let optimized_auc = Auc.evaluate(&preds, &labels, None, 1);
    ...
    fn weighted_auc_emphasizes_pairs() {
        ...
        let unweighted = Auc.evaluate(&preds, &labels, None, 1);
        assert!((unweighted - 1.0).abs() < 1e-10);

        // Equal weights should also give 1.0
        let weights = vec![1.0, 1.0, 1.0, 1.0];
        let weighted = Auc.evaluate(&preds, &labels, Some(&weights), 1);
        ...
        let weighted_zero = Auc.evaluate(&preds, &labels, Some(&weights_zero), 1);
    ...
    fn mlogloss_perfect() {
        ...
        let mlogloss = MulticlassLogLoss.evaluate(&preds, &labels, None, 3);
    ...
    fn weighted_mlogloss() {
        ...
        let preds = vec![
            0.9, 0.1, // class 0: sample 0, sample 1
            0.05, 0.8, // class 1: sample 0, sample 1
            0.05, 0.1, // class 2: sample 0, sample 1
        ];
        let labels = vec![0.0, 1.0];
        ...
        // Unweighted: mean of two losses
        let unweighted = MulticlassLogLoss.evaluate(&preds, &labels, None, 3);
        ...
        let weighted = MulticlassLogLoss.evaluate(&preds, &labels, Some(&weights), 3);
